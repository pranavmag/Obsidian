...
Actually yeah, I forgot to consider the index probe which means that it would be 1001 I/Os for using the index and 1000 I/Os still for the full table scan. That doesn't seem like much but when a full table scan is done, it goes through one by one in a sequential continuous path whereas using the index would cause the database to jump randomly throughout the disk. Even if we assume that multiple tuples we need can be stored on the same page, the pessimistic upper bound possibility is still there. 